...
# ----------------------------------------------------------------------------
# 文件名: {删除无标签图片}.py
# 功能: {使用X-AnyLabeling等软件进行数据标注后，可能由于误操作，导致删除掉了标签，但是对应图片还存在的情况。这时候运行程序，就可以删除多余的图片}
# 作者: {Emilison_Black}
# 创建日期: {2025年2月15日}
# 最后修改日期: {2025年2月17日}
# 版本: {V1.0}
...
# 请在这里设置图片和标签文件夹的路径
IMAGE_DIR = r'打标文件/images/高质量图片'  # 替换为你的图片文件夹路径
LABEL_DIR = r'打标文件/images/高质量图片/high_quality_labels'  # 替换为你的标签文件夹路径
...
def soft_delete_orphan_images(image_dir, label_dir, trash_dir):
    ...
    print(f"清理完成。移动了 {len(orphan_images)} 个无对应标签的图片文件到回收站。")
